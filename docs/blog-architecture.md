...
| `services/blog/generateDraft.ts` (`proposeNextActualiteBestEffort`) | Notifie Discord de chaque candidat indépendamment (best-effort par candidat — l'échec de l'un n'empêche jamais les autres). Appelée **uniquement** par `runVeilleScan()` (scan dédié, cf. `.github/workflows/blog-veille-trigger.yml`) — plus jamais par `generateDraft()` lui-même (retiré le 2026-09-10 : un déclenchement manuel de la génération hebdomadaire produisait sinon un second lot de propositions Discord le même jour que le scan dédié) |
